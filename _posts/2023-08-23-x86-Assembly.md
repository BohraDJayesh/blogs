...
< After you've read [Instructions](#instructions) and [ClockSpeedAndClockCycle](#clock-speed-and-clock-cycle) sections come back here and you will understand even more. ><br>
...
![Register-Arch](/blogs/assets/BlogImages/Different-stor-registers.png)
...
What we generally have been told is **" A Higher Clock Speed means a faster CPU "**, I Somewhat agrees to that point. Why **SOMEWHAT** ? and what does it really means ? <br>
Now that we've learned a bit about processors, their instructions, and how they work, we can dive into some technical details. So clock speed is a way to measure cpu's performance or we can say that the clock speed is a measure of how fast a cpu can process or executes instructions.
<br>If we look at the official definations from intel blog -<br>
***<small>The clock speed measures the number of cycles your CPU executes per second, measured in GHz (gigahertz).</small>***<br>
<small>*Source - Intel*</small><br>
A blog of Yale University explains it using a great and simple example, you can refere it [here](#) 

```assembly
"Students have been sitting in class for a long time. Finally the bell rings throughout the school signaling the end of the
current period. Everyone gets up and moves through the hall to their next classroom. After a few minutes the bell rings again to
signal the start of the next period. The bell has to ring everywhere in the school at the same time to coordinate movement.
Without the bell, some classes would be released early and others would be released late."
```

Older CPUs were not that capable as of they are now. What they do is they followed "Fetch-Decode-Execute-Store" Cycle each per cycle. So during each and every clock cycle one instruction executes that means that a CPU with 4Ghz clock speeds can executes 4 Billion cycles per seconds running on each core. Woah !! So just imagine just a 4 core intel pentium 4 cpu with just 2 Ghz clock speed can executes how much of instructions during each cycle ? Whatever it is despite being a big number, it's still isn't a lot.
![FetchCycle](/blogs/assets/BlogImages/FetchCycle.png)

In Simple terms to perfom execution it firsts fetches the instruction, decode it's operator and operand, executes it and then goes towards storing returned value. So during older times it keeps repeating, they fetch-decode-execute-store then again fetch-decode-execute-sotre and so on. So it took **"4"** Clock cycles to do one instruction !!, which results in the instruction per clock cycle were less and required more time to do a normal task.

![InstrcutionClockCycle](/blogs/assets/BlogImages/InstructionClockCycle.png)

What modern processors allows now saves a lot of time and increases efficiency in many ways. Unlike old times, while the processor is fetching instruction, then decoding it, at the same time or during same clock cycle it can start fetching another instruction , parelling executing the previous one, fethcing the third one all in one clock cycle, thus saving a lot of time.

![NewInstructionCycle](/blogs/assets/BlogImages/NewInstructionCycle.png)

They've even advanced this for example if one of the stage is consuming longer time frame, they can be broken down into smaller stages as well. This all has been possible because of the introduction of threads and multi threading. One thing to note here is not all instructions takes the same time or same clock cycle to execute, it totally depends on the complexity and type of instruction being executed. For example add 0, is not gonna take much time than add 83434, 3435345.

**One important thing I would like to address here is difference between clock cycles of risc and cisc architecture.**
While CISC have more complex instruction of various lengths, the clock cycle between certain instruction can differ but in RISC architecture, as all the instructions are of same length as each one of them is the most basic one possible - during each clock cycle because of same length instruction can execute it posses higher clock speed and higher clock cyle comparitive to CISC Architecure.

![CISCArchitecture](/blogs/assets/BlogImages/CISCClockCycle.png)

![RISCArchitecutre](/blogs/assets/BlogImages/RISCClockCycle.png)

Now I think we've a lot more clearity between [RISC and CISC](#risc-vs-cisc-architectures), and now if you read the article again, you can understand difference between them a lot clear now !!.<br>

### Assembly File Structure
To code in assembly it's file structure follows a pattern and is mainly is divided into 4 sections - 
 - Directives
 - Labels
 - Instructions
 - Operand

We've already talked about instructions and operand, talking about the reamining ones we've to include .data and .text in each assembly code. .text is the section in which we write our main code into. _start is the label we generally use to instruct the compiler where the program is being started, we call it using global _start .<br>
The .data section contains all our variables informations and their memory segments as well. We can define the variable usind db, dw and dd which represents - bytes, words and digits for our respective variable data type. We will dive more deeply in it once we begin to write the code in assembly, so let's begin.<br>
...
_start:
    ; In the .text section we have declare an entry point _start to where the actual execution begins.
    mov eax, 0x4 ; In x86 to make system call we have to assign a number to accumulator and to call write function its 4.
...
; And voilla ! We have written our first "Hello World" program in assembly language.
```
...
```bash
section .data
    integer dd 53483    ; random integer to convert it into its ascii form.
...
convertLoop:
    ...
    mov [edi], dl    ; Store the ASCII character in the buffer, here dl is the lower bits of edx register. 
                     ; We are not using edx because then the '0's ascii value will be added to whole register and will output something else.
    ...
    ; Now to calculate the length of the ASCII representation first we will get the value of current edi, move it to eax and will calculate how much it is from output, hence getting the length of output.
...
```bash
section .data
    divident dd 17 ; Just for example we are taking divident as 17.
...
_start:
    ...
    ; Now for displaying the result we have to convert our registers data into ascii form.
    ; for that we will be creating a function called itoa, it's just like any other label but we're naming it here function.
...
```bash
VI.Making a right star pyramind pattern in assembly language. ( I swear this is the last one, promise !!!)** --
VII. Making a left star pyramind in assembly.                                                                 | -- Your Homework
VII. Making a centered star pyramind in assembly.                                                            --
```

### Advance
Honestly, I've covered a handfull of topics in assembly, and for now, it might be beyond my current capabilities. I believe this should give you a strong start in your journey into reverse engineering and exploring x86 assembly language and now you are capable enough to travel alone from here my fellow adventurer !!!. **BUT**<br><br>
We will see about that in our next blog post on Malwares and Assembly 😉, so for now 👋👋👋👋.<br>
**PS** - You can find all of my program in my [Gist Repo](https://gist.github.com/BohraDJayesh) !!.
