...
## Hello World
Let's start with our first program, we will make a file in terminal and will name it as hello_world.asm - 
### Begineer
i. The first program we're going to built is ofcourse the one above all !! "Hello World !!".
...
**ii.** The next program we're going to do is Converting integer into it's ascii representation so that we can print it in the output screen. It's quite complex program but we'll learn a lot from this.
...
**iii.** The next program we're going to do is finding modulus of a number. I request you to try it yourself first and then move further.
...
$ vim mod.asm
```
Now before writing the program let's explore the logic we're going to apply. To find the modulo
```assembly
section .data
...
**iv.** Let's take it a step further, the next program we're going to do is even number identifier. I request you to try it yourself first and then move further.
...
### Intermediate
In the Intermediate section we will start directly from making a calculator program that takes input from the user what it has to perform and then based on that it performs several actions.
